...
void remove_bad_cluster(disk *disk_ptr) {
  ...
  for (no = 2; no < disk_ptr->cluster.num; ++no) {
    if (disk_ptr->FAT.buf[no].badsec == FAT_BADSEC_OK) continue;
    if (disk_ptr->FAT.buf[no].value == 0xfff7) continue;
    prerr_int("クラスタ %04X は、使用不可能です", "cluster %04X is unusable",
              no);
...
void read_FAT(disk *disk_ptr) {
  ...
  if (!flags.force) {
    ...
    for (ctr = 0; ctr < disk_ptr->FAT.sector_num; ++ctr) {
      if (is_bad_sector(disk_ptr, no)) ok = 0;
      ++no;
    }
    if (!ok) {
  ...
  dummy = (unsigned char *)Malloc(disk_ptr->sector.length *
                                  disk_ptr->FAT.sector_num);
  if (diskred(disk_ptr, dummy, disk_ptr->FAT.top_sector,
              disk_ptr->FAT.sector_num)) {
    prerr_("ＦＡＴは読み書き出来ません", "FAT is not readable");
    cleanup_exit(1);
  }
  ...
  if (disk_ptr->FAT.is_2bytes) {
    ...
    for (no = 0; no < disk_ptr->cluster.num; ++no) {
      ...
      if (dst->value > 0xff6) dst->value |= 0xf000;
      ++dst;
      if (++no == disk_ptr->cluster.num) break;
...
void write_FAT(disk *disk_ptr, unsigned short cluster) {
  ...
  if (disk_ptr->FAT.is_2bytes) {
    ...
    if (to > disk_ptr->cluster.num) to = disk_ptr->cluster.num;
    if (disk_ptr->FAT.little_endian) {
...
static void check_FAT_outof(disk *disk_ptr, int first_time) {
  ...
  for (no = 2; no < disk_ptr->cluster.num; ++no) {
    if (disk_ptr->FAT.buf[no].badsec == FAT_BADSEC_YES) continue;
...
static void check_FAT_loop(disk *disk_ptr, int first_time) {
  ...
  for (no = 2; no < disk_ptr->cluster.num; ++no) {
    ...
    if (disk_ptr->FAT.buf[no].loop != FAT_LOOP_UNDECIDED) continue;
    last = test = no;
    while (1) {
      unsigned short fill;
      ...
      for (test = no; disk_ptr->FAT.buf[test].loop == FAT_LOOP_IN_SEARCH;
           test = disk_ptr->FAT.buf[test].value) {
...
int expected_length(disk *disk_ptr, unsigned short cluster, int length) {
  ...
  cluster_num = 1;
  while (1) {
    if (disk_ptr->FAT.buf[cluster].type != FAT_TYPE_LINKING) break;
