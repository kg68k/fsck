  ...
  if (flags.english)
    {
      ...
      printf("\t-debug-force-2bytes\trecognize that FAT has 2bytes size (automatic decision).\n");
      printf("\t-debug-force-is1.5bytes\t\t  that FAT has 1.5bytes size (automatic decision).\n");
      printf("\t-debug-path\t\tdisplay path (don't display),\n");
      printf("\t-debug-sector\t\tthe verbose mode of sector reading/writing (don't display),\n");
#endif
      printf("\t-english\t\tthe english mode (japanese mode),\n");
      printf("\t-force\t\t\tforce to read/write (not force),\n");
      printf("\t-ignore-archive-attrib\tignore the archive attribute (don't ignore),\n");
#else
      printf("\t-english\t\tthe english mode (default japanese mode),\n");
#endif
#ifdef DETERMINE_LITTLE_ENDIAN_BY_OPTION
      printf("\t-isLittleEndian\t\trecognize that FAT is little endian (big endian),\n");
#endif
      printf("\t-is2bytes\trecognize that FAT has 2bytes size (automatic decision),\n");
      printf("\t-is1.5bytes\t\t  that FAT has 1.5bytes size (automatic decision),\n");
#ifdef FSCK
  ...
  else
    {
      ...
      printf("\t-debug-force1.5bytes\tFAT 1.5バイトサイズの強制指定（自動判断）\n");
      printf("\t-debug-path\t\tパスの表示を行なう（表示しない）\n");
      printf("\t-debug-sector\t\tセクター読み書きおしゃべりモード（表示しない）\n");
#endif
      printf("\t-english\t\t英語表示にする（日本語表示）\n");
      printf("\t-force\t\t\t無理に読み書きする（無理に読み書きしない）\n");
      printf("\t-ignore-archive-attrib\t属性 A がなくても通常ファイルとみなす（みなさない）\n");
#else
      printf("\t-english\t\t英語表示にする（デフォルトは日本語表示）\n");
#endif
#ifdef DETERMINE_LITTLE_ENDIAN_BY_OPTION
      printf("\t-isLittleEndian\t\tFAT は PC コンパチである (Human オリジナル),\n");
#endif
...
int main(int argc, char *argv[])
{
  ...
  if ((int)ptr < 0)
	...
	print_("仮想デバイスはサポートしていません", "We support of no virtual devices");
      else
  ...
  B_PRINT("デバッグバージョンは FAT サイズ決定の確認を行ないます.\r\n");
      ...
      if (flags.writing)
	{
	  ...
	  B_PRINT(".\r\n一度書き込み無し（-writing をつけない）でチェックすると良いです");
	  while ((c = keyinp()) != '\r')
	    {
	      if (c == 3)
		{
