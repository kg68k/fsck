...
static inline int is_directory(int atr)
{
  return (atr & 0x10) != 0;
}
...
static inline int is_symlink(int atr)
{
  return (atr & 0x40) != 0;
}
...
static inline int is_volume(int atr)
{
  return (atr & 0x08) != 0;
}
...
extern directory *subdir(disk *disk_ptr, const char *dir_path, unsigned short cluster);
