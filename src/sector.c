...
void check_read_sector(disk *disk_ptr) {
  ...
  if (!flags.check_reading_sectors) return;
  if (flags.writing && flags.check_writing_sectors) return;
  set_error_mode(1);
...
void check_write_sector(disk *disk_ptr) {
  ...
  while (cur_sector < max_sector) {
    ...
    if (cur_sector + check_length > max_sector)
      check_length = max_sector - cur_sector;
    for (offset = 0; offset < check_length; ++offset)
      if (is_bad_sector(disk_ptr, cur_sector + offset)) check_length = offset;
    if (check_length == 0) {
      ++cur_sector;
      continue;
    }
    ...
    if (diskred(disk_ptr, dummy, cur_sector, check_length) || ({
          ptr = dummy;
          ptr2 = dummy2;
          for (offset = 0; offset < check_length * disk_ptr->sector.length;
               ++offset)
            *ptr2++ = *ptr++ ^ 0xff;
          0;
        }) ||
...
int set_bad_sector(disk *disk_ptr, int sector_offset) {
  ...
  if (disk_ptr->sector.num <= sector_offset) return ERROR_OUT_OF_SECTOR;
...
int is_bad_cluster(disk *disk_ptr, int cluster_offset) {
  ...
  for (cur = from; cur < to; ++cur)
    if (is_bad_sector(disk_ptr, cur)) return 1;
  return 0;
}
...
void read_sector(void *buffer, disk *disk_ptr, int from, int num) {
  ...
  while (from < to) {
    for (real_to = from; real_to < to; real_to++)
      if (is_bad_sector(disk_ptr, real_to)) break;
    if (from < real_to) {
...
void read_cluster(void *buffer, disk *disk_ptr, unsigned short from,
                  unsigned short num) {
  read_sector(
      buffer, disk_ptr,
      (from - 2) * disk_ptr->cluster.sectors_per + disk_ptr->data.top_sector,
...
void write_sector(const void *buffer, disk *disk_ptr, int from, int num) {
  ...
  while (from < to) {
    ...
    if (from < real_to) {
      ...
      prerr_int("セクタ %d に書き出し出来ません", "can not write to sector %d",
                from);
      end_line();
      from++;
    }
...
void write_cluster(const void *buffer, disk *disk_ptr, unsigned short from,
                   unsigned short num) {
  if (!flags.writing) return;
  write_sector(
      buffer, disk_ptr,
