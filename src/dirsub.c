...
static int is_illegal_code(int data, int *last_is_kanji) {
  data &= 0xff;
  if (0 <= data && data <= 0x20) return 1;
  if (data == '?' || data == '*' || data == 0x7f) return 1;
  if (*last_is_kanji) {
    ...
    if (iskanji(data)) *last_is_kanji = data;
  }
  return 0;
}
...
static int is_legal_filename(direntry *entry) {
  ...
  last_is_kanji = 0;
  if (is_illegal_code((data == 5) ? 0xe5 : data, &last_is_kanji)) return 0;
  for (offset = 1; offset < 8; ++offset) {
    data = (unsigned char)entry->fileName[offset];
    if (data == ' ') {
      if (is_illegal_code('1', &last_is_kanji)) return 0;
      while (++offset < 8)
        if (entry->fileName[offset] != ' ') return 0;
      /* 先頭が $00 なら良しとする(Windows の拡張ファイルエントリ対策) */
      return (entry->fileName2[0] == 0) ? 1 : 0;
    }
    if (is_illegal_code(data, &last_is_kanji)) return 0;
  }
  for (offset = 0; offset < 10; ++offset) {
    ...
    if (data == 0) {
      ...
      while (++offset < 10)
        if (entry->fileName2[offset] != 0) return 0;
      return 1;
    }
    if (is_illegal_code(data, &last_is_kanji)) return 0;
  }
...
static int is_legal_attr(direntry *entry) {
  ...
  attr = entry->atr;
  if (is_volume(attr)) {
    if (is_directory(attr)) return 0;
    if (is_file_or_symlink(attr)) return 0;
    return 1;
  }
...
static int _nextentry(disk *disk_ptr, directory *ptr) {
  ...
  if (ptr->entry->fileName[0] == 0) {
    if (ptr->offset + sizeof(direntry) < ((ptr->sector == -1)
                                              ? disk_ptr->cluster.length
                                              : disk_ptr->sector.length))
      ptr->entry[1].fileName[0] = 0;
    return NEXT_ENTRYEND;
  }
  if (ptr->entry->fileName[0] == (char)0xe5) return NEXT_DELETED;
  if (ptr->dir_path) ptr->file_path[0] = 0;
  if (is_legal_filename(ptr->entry))
    if (is_legal_ext(ptr->entry)) {
      ...
      if (is_legal_attr(ptr->entry)) return NEXT_NORMALENTRY;
    }
  if (ptr->file_path[0] == 0) {
    prerr_str("ディレクトリ %s の ", "directory %s has illegal ",
              ptr->dir_path);
    prerr_int("%d 番目に不正エントリが有ります", "%d th entry",
              ptr->total_offset + 1);
  } else
    prerr_str("%s は不正エントリです", "%s is an illegal entry",
              ptr->file_path);
...
int nextentry(disk *disk_ptr, directory *ptr) {
  while (1) {
    ...
    if (flag == NEXT_BUFEND) break;
    if (flag == NEXT_ENTRYEND) break;
    if (flag == NEXT_IGNORED) continue;
    if (flag == NEXT_DELETED) continue;
#ifdef DEBUG
    if (debug.path) {
      print_str("[%s]", SAME, ptr->file_path);
      if (!debug.link) end_line();
    }
#endif
    return 1;
  }
...
int nextfreeentry(disk *disk_ptr, directory *ptr) {
  ...
  while (1) {
    ...
    if (flag == NEXT_BUFEND) break;
    if (flag == NEXT_IGNORED) continue;
    if (flag == NEXT_NORMALENTRY) continue;
    strcpy(ptr->file_path, ptr->dir_path);
    strcat(ptr->file_path, "/");
    return 1;
  }
