...
void collect(directory *dir)
{
  ...
  dir = copyentry(dir);
  for (num = 0, ptr = collection_buffer; num < collection_num; ++num, ++ptr)
    if (strcmp((*ptr)->file_path, dir->file_path) == 0)
      return;
  if (collection_num >= buffer_length)
    {
      ...
      collection_buffer = (directory **)Malloc(sizeof(directory *) * buffer_length);
      if (work)
	{
	  int offset;
...
void adjust(disk *disk_ptr, directory *dir)
{
  ...
  for (num = 0, ptr = collection_buffer; num < collection_num; ++num, ++ptr)
    {
      if (dir->sector != (*ptr)->sector)
	continue;
      if (dir->sector == -1)
	if (dir->cluster != (*ptr)->cluster)
	  continue;
      if ((dst = (*ptr)->buffer) == NULL)
	continue;
      src = dir->buffer;
  ...
  for (num = 0, ptr = collection_buffer; num < collection_num; ++num, ++ptr)
      ...
      while (1)
	{
	  if (disk_ptr->FAT.buf[cluster].refered == FAT_REFERED_MANY)
	    if (cluster > batting)
	      batting = cluster;
	  if (disk_ptr->FAT.buf[cluster].refer != FAT_REFER_OK)
	    break;
...
static void sort_by_batting(directory **collection_buffer, int collection_num)
{
  ...
  if (collection_num == 2)
  ...
  else
    {
      ...
      top_batting = collection_buffer[0]->batting_cluster_max;
      middle_batting = collection_buffer[(collection_num - 1) / 2]->batting_cluster_max;
...
int its_duplicators(directory **dir_ptr)
{
  ...
  if (duplication_offset == collection_num)
    return 0;
...
{
  if (shipout_offset == collection_num)
    return 0;
