...
disk *init_disk(int drive) {
  ...
  work->drive_no = drive;
  if (GETDPB(drive + 1, &dpbbuf) < 0) return (disk *)ERROR_VIRTUAL_DEVICE;

  work->sector.length = dpbbuf.byte;
  work->FAT.top_sector = dpbbuf.fatsec;
  work->FAT.sector_num = dpbbuf.fatlen;
  work->root.top_sector =
      work->FAT.top_sector + dpbbuf.fatcount * work->FAT.sector_num;
  ...
  work->sector.bads =
      (unsigned char *)Malloc(sizeof(unsigned char) * table_num);
  for (offset = 0; offset < table_num; ++offset) work->sector.bads[offset] = 0;
