...
static int key_handler() {
  ...
  if (error_seclen == -1)
    ...
    if (error_seclen != 1)
      sprintf(errmsg, " (Human sector %08X ～ sector %08X)", error_sector,
              error_sector + error_seclen - 1);
    else
...
static void doserrret(void) { cleanup_exit(255); }
...
void set_error_mode(int ignore) { ignore_flag = ignore ? 1 : 0; }
