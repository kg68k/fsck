...
int keyinp() {
  ...
  if ((code & 0xff00) == 0x7100) {
    while ((code = B_KEYINP() & 0xff) == 0)
      ;
    if (0x20 <= code && code < 0x40) code -= 0x20;
  }
  return code & 0xff;
}
...
const char *jore(const char *jstring, const char *string) {
  if (string == SAME) return jstring;
  if (jstring == SAME) return string;
...
void prerr_attribute(unsigned char attribute) {
  print_pre("    *** ");
  printf((attribute & 0x40)
             ? "l"
             : ((attribute & 0x10) ? "d" : ((attribute & 0x08) ? "v" : "-")));
...
void end_line(void) { in_record = END_LINE; }

void end_record(void) {
  if (in_record != END_LINE) in_record = END_RECORD;
}
