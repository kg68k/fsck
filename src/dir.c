...
static void check_subdir(disk *disk_ptr, directory *dir) {
  while (nextentry(disk_ptr, dir)) {
    ...
    entry = dir->entry;
    if (entry->fileName[0] == '.') {
      if (entry->fileName[1] == ' ') continue;
      if (entry->fileName[1] == '.' && entry->fileName[2] == ' ') continue;
    }
    ...
    if (is_file_or_directory(entry->atr)) {
      ...
      while (1) {
        if (file_cluster < 2 || disk_ptr->cluster.num <= file_cluster ||
          ...
          prerr_str("ファイル %s には実体が有りません", "file %s has no body",
                    dir->file_path);
          if (flags.writing) {
            end_record();
...
void find_lost_files(disk *disk_ptr) {
  ...
  if (flags.writing) {
    ...
    while (nextentry(disk_ptr, root)) {
      entry = root->entry;
      if (strncmp(entry->fileName, "lost+fou", 8)) continue;
      if (strcmp(entry->fileName2, "nd")) continue;
        ...
        if (disk_ptr->FAT.buf[no].refered == FAT_REFERED_NO &&
          ...
          if (flags.writing) {
            ...
            if (next) {
              ...
              for (offset = 0; offset < 3; ++offset) entry->ext[offset] = ' ';
              entry->atr = 0x20;
              ...
              end_record();
              prerr_(
                  "lost+found 及び root にこれ以上領域が無いため、"
                  "復元出来ませんでした",
  ...
  if (flags.writing) freedir(lost_found);
  end_section();
}
  ...
  while (nextentry(disk_ptr, dir)) {
    ...
    if (is_file_or_directory(entry->atr)) {
      ...
      while (1) {
        ...
        if (disk_ptr->FAT.buf[file_cluster].refer != FAT_REFER_OK) break;
        file_cluster = disk_ptr->FAT.buf[file_cluster].value;
      }
#ifdef DEBUG
      if (debug.link) {
    ...
    if (is_directory(entry->atr)) {
      ...
      if (disk_ptr->FAT.buf[child_cluster].subdir == FAT_SUBDIR_NOTYET) {
        ...
        collect_bad_direntry(disk_ptr, child_dir, child_datetime, child_cluster,
                             child_attribute, my_datetime, my_cluster,
                             my_attribute);
        freedir(child_dir);
      } else {
        prerr_str("ディレクトリ %s はループしています",
    ...
    if (is_file_or_symlink(entry->atr)) {
      ...
      if (real_length != file_length) collect(dir);
    }
  }
}
...
static void fix_file_length(disk *disk_ptr, directory *dir) {
  ...
  readentry(disk_ptr, dir);
  entry = dir->entry;
  if (is_directory(entry->atr)) return;
  file_cluster = get_cluster(entry);
  file_length = get_length(entry);
  real_length = expected_length(disk_ptr, file_cluster, file_length);
...
void fix_files(disk *disk_ptr) {
...
#endif
  {
    ...
    for (no = 2; no < disk_ptr->cluster.num; ++no)
      disk_ptr->FAT.buf[no].subdir = FAT_SUBDIR_NOTYET;
    collect_bad_direntry(disk_ptr, root, datetime, 0, attribute, datetime, 0,
                         attribute);
    freedir(root);
    while (is_there_more_duplication(disk_ptr, &dir)) {
      warning_duplication(disk_ptr, dir);
      while (its_duplicators(&dir)) fix_duplication(disk_ptr, dir);
    }
    while (next_collection(&dir)) fix_file_length(disk_ptr, dir);
  }
  end_section();
}
