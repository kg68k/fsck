...
void check_root_sub(disk *disk_ptr, directory *root, unsigned long datetime,
                    int second_char) {
  direntry *entry;
  int next;
  ...
  if (next) {
    ...
    for (offset = 2; offset < 8; ++offset) entry->fileName[offset] = ' ';
    for (offset = 0; offset < 10; ++offset) entry->fileName2[offset] = 0;
...
void check_root(disk *disk_ptr) {
  ...
  while (nextentry(disk_ptr, root)) {
    ...
    if (entry->fileName[1] != '.' && entry->fileName[1] != ' ') continue;
    if (entry->fileName[2] != ' ') continue;
    if (flags.unix_like) {
      if (entry->fileName[1] == ' ')
        have_period = 1;
      else
    ...
    if (entry->fileName[1] == '.') prerr_(".", SAME);
  ...
  if (have_period == 0) {
    ...
    if (flags.writing) check_root_sub(disk_ptr, root, datetime, ' ');
    end_line();
  }
  if (have_period == 0) {
    prerr_("ルートにエントリ `..' が有りません",
